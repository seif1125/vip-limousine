...
const InquiryForm = ({ isModal = false, formData, onChange, onSubmit, onEmailSubmit, isAr, isProcessing }) => (
  <div className={`${!isModal ? 'bg-white/5 p-6 rounded-2xl border border-white/10 shadow-2xl' : ''}`}>
    <h4 className="text-white font-black text-xs uppercase tracking-[0.2em] mb-4">
      {isAr ? "استفسار الشركات" : "Company Inquiry"}
    </h4>
    <form className="space-y-3" onSubmit={onSubmit}>
      <div className="relative">
        <Building2 size={12} className={`absolute ${isAr ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-slate-500`} />
        <input 
          required name="company" type="text" placeholder={isAr ? "اسم الشركة" : "COMPANY NAME"}
      ...
      <div className="relative">
        ...
        <input 
          required name="name" type="text" placeholder={isAr ? "اسمك" : "YOUR NAME"}
          value={formData.name}
          onChange={onChange}
          className={`w-full bg-[#0F172A] border border-white/10 rounded-lg ${isAr ? 'pr-10' : 'pl-10'} pr-4 py-3 text-[10px] font-bold tracking-widest text-white focus:outline-none focus:border-[#C5A25D] transition-colors`}
        />
...
export default function Footer({ settings }) {
  ...
  const handleWhatsAppRedirect = (e) => {
    e.preventDefault();
    const text = isAr 
      ? `استفسار من شركة - في آي بي ليموزين مصر\n\nالشركة: ${formData.company}\nالاسم: ${formData.name}\nالبريد الإلكتروني: ${formData.email}\nالرسالة: ${formData.message}`
      : `Company Inquiry - VIP Limousine Egypt\n\nCompany: ${formData.company}\nContact Name: ${formData.name}\nEmail: ${formData.email}\nMessage: ${formData.message}`;
        ...
        <div className="max-w-7xl mx-auto px-6 pb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-center md:text-right">
          ...
          <div className="lg:border-l border-slate-700 md:px-6">
            ...
            <div className="space-y-6">
              ...
              <Link href={`https://wa.me/${settings.phones.whatsapp1.replace(/\D/g, '')}`} className="flex items-center gap-4 justify-center md:justify-start group">
                 <MessageSquare size={18} className="text-[#C5A25D]" />
                 <span className="text-sm font-black">{isAr ? "واتساب لخدمة العملاء" : "WhatsApp Concierge"}</span>
              </Link>
            </div>
          </div>
